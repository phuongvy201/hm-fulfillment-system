...
@section('title', 'Create Exchange Rate - ' . config('app.name', 'Laravel'))

@section('header-title', 'Create New Exchange Rate')
@section('header-subtitle', 'Add a new exchange rate to the system')
...
<div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <form method="POST" action="{{ route('admin.currencies.store') }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        @csrf
        ...
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="from_currency" id="from_currency" required
                        ...
                        <option value="{{ $currency->code }}" {{ old('from_currency') == $currency->code ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="to_currency" id="to_currency" required
                        ...
                        <option value="">-- Select currency --</option>
                        @foreach($currencies as $currency)
                        <option value="{{ $currency->code }}" {{ old('to_currency') == $currency->code ? 'selected' : '' }}>
                            {{ $currency->code }} - {{ $currency->name }} ({{ $currency->symbol }})
                        </option>
                        @endforeach
                    </select>
            ...
            <div>
                ...
                <div class="relative">
                    <input type="number" 
                        ...
                        required
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    <p class="mt-1 text-xs text-gray-500">Example: 1 {{ old('from_currency', 'USD') }} = <span id="rate-preview">{{ old('rate', '0') }}</span> {{ old('to_currency', 'VND') }}</p>
                </div>
            </div>
            ...
            <div>
                ...
                <textarea name="notes" 
                    ...
                    rows="3"
                    class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
                    placeholder="Add notes about this exchange rate (e.g. this week's exchange rate from bank X)...">{{ old('notes') }}</textarea>
            </div>
