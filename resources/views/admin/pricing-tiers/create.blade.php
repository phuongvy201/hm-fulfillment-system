...
@section('title', 'Create Pricing Tier - ' . config('app.name', 'Laravel'))

@section('header-title', 'Create Pricing Tier')
@section('header-subtitle', 'Add a new pricing tier')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white rounded-lg shadow-sm p-8">
        <form method="POST" action="{{ route('admin.pricing-tiers.store') }}">
            @csrf

            <div class="space-y-6">
                <div>
                    <label for="slug" class="block text-sm font-semibold mb-2">Slug</label>
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}" required placeholder="e.g., wood, silver, gold" class="w-full px-4 py-3 border rounded-lg">
                </div>

                <div>
                    <label for="name" class="block text-sm font-semibold mb-2">Name</label>
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required class="w-full px-4 py-3 border rounded-lg">
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mt-1">Higher priority = better tier</p>
                </div>

                <div>
                    <label for="description" class="block text-sm font-semibold mb-2">Description</label>
                    <textarea id="description" name="description" rows="4" class="w-full px-4 py-3 border rounded-lg">{{ old('description') }}</textarea>
                </div>
                        ...
                        <div id="autoAssignFields">
                            ...
                            <div>
                                ...
                                <select id="reset_period" name="reset_period" required class="w-full px-4 py-3 border rounded-lg">
                                    <option value="monthly" {{ old('reset_period', 'monthly') === 'monthly' ? 'selected' : '' }}>Hàng tháng</option>
                                    <option value="quarterly" {{ old('reset_period') === 'quarterly' ? 'selected' : '' }}>Hàng quý</option>
                                    <option value="yearly" {{ old('reset_period') === 'yearly' ? 'selected' : '' }}>Hàng năm</option>
                                    <option value="never" {{ old('reset_period') === 'never' ? 'selected' : '' }}>Không reset</option>
                                </select>
