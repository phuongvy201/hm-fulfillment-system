...
@section('title', 'Create Pricing Rule - ' . config('app.name', 'Laravel'))

@section('header-title', 'Create Pricing Rule')
@section('header-subtitle', 'Add a new pricing rule (e.g., t-shirt UK in 1 mặt +3.5)')
        ...
        <form method="POST" action="{{ route('admin.pricing-rules.store') }}">
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="rule_type" name="rule_type" value="{{ old('rule_type') }}" required placeholder="e.g., printing_sides, size_adjustment" class="w-full px-4 py-3 border rounded-lg">
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="condition_key" class="block text-sm font-semibold mb-2">Condition Key</label>
                        <input type="text" id="condition_key" name="condition_key" value="{{ old('condition_key') }}" placeholder="e.g., product_type, size, color" class="w-full px-4 py-3 border rounded-lg">
                    </div>

                    <div>
                        <label for="condition_value" class="block text-sm font-semibold mb-2">Condition Value</label>
                        <input type="text" id="condition_value" name="condition_value" value="{{ old('condition_value') }}" placeholder="e.g., t-shirt, S, red" class="w-full px-4 py-3 border rounded-lg">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select id="operation" name="operation" required class="w-full px-4 py-3 border rounded-lg">
                        ...
                        <option value="set" {{ old('operation') === 'set' ? 'selected' : '' }}>Set (=)</option>
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" id="amount" name="amount" value="{{ old('amount') }}" required step="0.01" class="w-full px-4 py-3 border rounded-lg">
                    </div>

                    <div>
                        <label for="currency" class="block text-sm font-semibold mb-2">Currency (Optional)</label>
                        <input type="text" id="currency" name="currency" value="{{ old('currency') }}" maxlength="3" placeholder="USD" class="w-full px-4 py-3 border rounded-lg uppercase">
                    </div>
                </div>

                <div>
                    <label for="priority" class="block text-sm font-semibold mb-2">Priority</label>
                    <input type="number" id="priority" name="priority" value="{{ old('priority', 0) }}" min="0" class="w-full px-4 py-3 border rounded-lg">
                    <p class="text-xs text-gray-500 mt-1">Higher priority = applied first</p>
                </div>
